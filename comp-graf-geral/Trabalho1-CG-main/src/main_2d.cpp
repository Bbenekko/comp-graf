...
static void initialize (void)
{
  ...
  // background photo
  //auto background = Node::Make(trf_background, {Texture::Make("sun", "images/space.jpg"), Color::Make(1, 1, 1)}, {Quad::Make()});
  auto trf_background = Transform::Make();
  trf_background->Scale(25.0f, 10.0f, 1.0f);  
  trf_background->Translate(-0.4f, 0.0f, 0.0f);
  auto background = Node::Make(trf_background, {Texture::Make("background", "images/espaco.jpg"), Color::Make(1, 1, 1)}, {Quad::Make()});
  ...
  auto sun = Node::Make(trf_sun, {Color:: Make(1, 1, 1), Texture::Make("tex", "images/sol.jpg")},{Disk::Make()});
  ...
  auto root =  Node::Make(shader, {background, sun, earth_orbit_disk, mercury_orbit_disk});
  scene = Scene::Make(root);
  scene->AddEngine(Orbit::Make(trf_mercury_orbit_disk, 4.15f));
  scene->AddEngine(Orbit::Make(trf_earth_orbit_disk, 1.0f));
  scene->AddEngine(Orbit::Make(trf_earth, 20.0f));
